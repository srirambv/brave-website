...
var languages = fs.readdirSync(path.join(__dirname, '..', 'public', 'assets', 'locales')).filter(function (language) {
...
if (process.env.META) {
  var localLanguages = fs.readdirSync(path.join(__dirname, '..', 'public', 'assets', 'locales'))
...
const TEMPLATE = 'https://www.transifex.com/api/2/project/brave-website/resource/RESOURCE_SLUG/translation/LANG_CODE/?file'

// Retrieve resource names dynamically
var resources = fs.readdirSync(path.join(__dirname, '..', 'public', 'assets', 'locales', 'en-US')).map(function (language) {
  ...
  resources.forEach(function (resource) {
    ...
    request.get(URI, {
      ...
      if (error) {
        ...
        var directory = path.join(__dirname, '..', 'public', 'assets', 'locales', languageCode.replace('_', '-'))
        ...
        var filename = path.join(__dirname, '..', 'public', 'assets', 'locales', languageCode.replace('_', '-'), resource + '.properties')
